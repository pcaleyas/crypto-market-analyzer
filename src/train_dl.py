    ...
    with mlflow.start_run(run_name=approach_name):
        ...
        for train_index, test_index in tscv.split(X_df):
            ...
            if approach_name == "dl_regression":
                ...
                logger.info(f"Fold {fold} - Best threshold from Val: {best_thresh:.2f}")
                y_pred_prob = model.predict(X_test_seq, verbose=0).flatten()
                y_pred = (y_pred_prob > best_thresh).astype(int)
                
            total_ret, sharpe, mdd = calculate_financial_metrics(df_test_aligned, y_pred)
            logger.info(f"Fold {fold} | Total Return: {total_ret*100:.2f}% | Sharpe: {sharpe:.4f} | MDD: {mdd*100:.2f}%")
        ...
        # Optional: log the model for the last fold in MLflow
        # Note: Keras models need mlflow.keras.log_model
        # But we will just log the final one in run_dl_training to save space
...
def run_dl_training():
    ...
    mlflow.set_experiment("Crypto_DL_Models")
