...
- **Results**: Average Sharpe Ratio of `-0.2896`, Average Max Drawdown of `-28.06%`.
...
### 2. Baseline Model (Phase 3 - XGBoost)
- **Methodology**: Gradient Boosting Classifier (XGBoost) predicting the binary `Realistic_Target`, with `scale_pos_weight` used to handle class imbalance, similar to balanced class weights.
- **Results**: Average Sharpe Ratio of `0.1979`, Average Max Drawdown of `-33.31%`.
- **Considerations**: Significant improvement over Random Forest. XGBoost manages to capture more complex non-linear patterns within static days, achieving a positive Sharpe Ratio, although the drawdown remains high. It still lacks the temporal sequence memory of deep learning.

### 3. Deep Learning Classification with Class Weights
...
### 4. Deep Learning Classification with Dynamic Threshold (The Best Performer)
...
### 5. Deep Learning Regression (The Initial Recommendation)
